 ...
 */
public class ReadOnlyFilesystemZipStore extends ZipStore {
    private static final Logger logger = Logger.getLogger(ReadOnlyFilesystemZipStore.class.getName());
    ...
    public ReadOnlyFilesystemZipStore(@Nonnull StoreHandle handle) {
        super(handle);
        zipStorePath = underlyingStore.toPath(); // throws if not FilesystemStore
        String msg = String.format("New instance dev.zarr.zarrjava.store.ReadOnlyFilesystemZipStore with path: %s",
                zipStorePath.toString());
        logger.log(Level.INFO, msg);
    }

    // Convenience constructor for filesystem paths
    public ReadOnlyFilesystemZipStore(@Nonnull Path zipPath) {
        this(new FilesystemStore(zipPath.getParent()).resolve(
                zipPath.getFileName().toString()));
    }

    // Convenience constructor for string paths
    public ReadOnlyFilesystemZipStore(@Nonnull String zipPath) {
    ...
    public void set(String[] keys, ByteBuffer bytes) {
        throw new UnsupportedOperationException("ReadOnlyFilesystemZipStore does not support set operation.");
    }

    @Override
    public void delete(String[] keys) {
        throw new UnsupportedOperationException("ReadOnlyFilesystemZipStore does not support delete operation.");
    }
    ...
    public String toString() {
        return "ReadOnlyFilesystemZipStore(" + underlyingStore.toString() + ")";
    }
